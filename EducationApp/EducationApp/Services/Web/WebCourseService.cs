...
namespace EducationApp.Services.Web
{
    public class WebCourseService : BaseCachedWebService, ICourseService
    {
        ...
        public async Task UpdateCourseSessions(Course course, bool speculatively = false)
        {
            ...
            await FetchCourseSessionsAsync(course);
        }

        private async Task<Category> GetSubcategoryFromCacheAsync(int categoryId)
        {
            try
            {
        ...
        private async Task UpdateCategoryCoursesInCacheAsync(ICollection<Course> courses, int categoryId)
        {
            ...
            await
                Cache.InsertObject(GetCategoryCacheKey(categoryId), category,
                    TimeSpan.FromDays(Constants.Internet.DefaultCacheDays));
        }
